...
import TopicList from '@/Components/HelpCenter/TopicList';
      ...
      <main className="w-full md:w-2/4 px-4">
        <div className="p-6 bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg">
          ...
          <nav className="flex items-center text-sm font-medium text-gray-500 dark:text-gray-400 mb-4">
            ...
            <ChevronRightIcon className="h-5 w-5 mx-1 flex-shrink-0 text-gray-400" />
            <Link href={`/help/${category.slug}/${category.articles[0].slug}`} className="hover:text-gray-700 dark:hover:text-gray-200">
              {category.category}
            </Link>
          </nav>

          <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-6">{article.title}</h1>
