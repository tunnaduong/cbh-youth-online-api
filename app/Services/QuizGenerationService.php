...
class QuizGenerationService
{
   ...
   * @param  bool  $isCustomTopic  True when$topic is user's own free-text
  ...
  public function generate(int $count, string $difficulty, ?string $topic, string $grade, bool$isCustomTopic = false): array
  {
    ...
    // Keep asking for the remainder if short, up to MAX_TOPUP_ROUNDS
    for ($round = 0; count($result['questions']) < $count &&$round < self::MAX_TOPUP_ROUNDS; $round++) {$missing = $count - count($result['questions']);
      try {
        $topUp =$this->requestBatch($missing,$difficultyLabel, $result['topic'],$grade, $isCustomTopic,$keys, $result['topic']);$result['questions'] = array_merge($result['questions'],$topUp['questions']);
      } catch (\Throwable $e) {
    ...
    foreach ($keys as$apiKey) {
      for ($attempt = 0; $attempt < 2; $attempt++) {
        try {
          ...
          // Directly parse and return without calling verifyAndFilter()
